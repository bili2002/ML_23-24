...
Add a sigmoid calculation to the code in `task7.py`. Plot the values it takes from `-10` till `10`.
...
Using `matplotlib` plot how the loss changes with epochs.
...
Create a dataset and model for the `NAND` logic gate.
...
Create a Python class named `Xor` and store all parameters and biases of the `Xor` model. Train and evaluate the model using appropriate architecture.
...
Finish any leftover tasks from class.

**Acceptance criteria**

1. All remaining tasks are solved.
